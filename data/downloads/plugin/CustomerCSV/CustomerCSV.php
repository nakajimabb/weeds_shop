...
class CustomerCSV extends SC_Plugin_Base {
    ...
    copy(PLUGIN_UPLOAD_REALDIR . $arrPlugin['plugin_code'] . '/' . 'customer_csv.php',
         PLUGIN_HTML_REALDIR . $arrPlugin['plugin_code'] . '/' . 'customer_csv.php');
    ...
    function prefilterTransformCustomerCSV(&$source, LC_Page_Ex $objPage, $filename) {
            ...
            default:
                ...
                if (strpos($filename, 'customer/subnavi.tpl') !== false) {
                    $objTransform->select('ul', NULL, false)->appendChild(
                        file_get_contents($template_dir . 'customer_csv_navi.tpl'));
                }
                break;
        }
