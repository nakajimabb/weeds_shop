...
<!--{if count($arrErr) == 0 and ($smarty.post.mode == 'search' or $smarty.post.mode == 'delete' or $smarty.post.mode == 'statuschange')}-->
        ...
        <table class="list" id="products-search-result">
            ...
            <!--{section name=cnt loop=$arrProducts}-->
                ...
                <tr style="background:<!--{$arrPRODUCTSTATUS_COLOR[$status]}-->;">
                    ...
                    <td class="menu" rowspan="2">
						<label>
						<input type="radio" name="statusdata[<!--{$arrProducts[cnt].product_id}-->]" value="1"<!--{if $key == 1}--> checked<!--{/if}-->>
						公開
						</label><br>
						<label>
						<input type="radio" name="statusdata[<!--{$arrProducts[cnt].product_id}-->]" value="2"<!--{if $key == 2}--> checked<!--{/if}-->>
						非公開
						</label>
						<a  class="btn-action" href="<!--{$smarty.const.ROOT_URLPATH}-->" onclick="fnChangeAction('?'); fnModeSubmit('statuschange', 'product_id', <!--{$arrProducts[cnt].product_id}-->); return false;" >
						<span class="btn-next">変更</span>
						</a>
					</td>
